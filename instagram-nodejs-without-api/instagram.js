...
module.exports = class Instagram {
  ...
  getUserDataByUsername(username) {
    ...
    return fetch("https://www.instagram.com/" + username, fetch_data).then(
      res =>
  ...
  */
  isPrivate(username) {
    return this.getUserDataByUsername(username).then(
      data => data.user.is_private
    );
  }
    ...
    return fetch("https://www.instagram.com/query/", {
      ...
      return res
        ...
        .then(function(response) {
          ...
          if (json.status == "ok") {
            self.userIdFollowers[userId] = self.userIdFollowers[userId].concat(
              json.followed_by.nodes
            );
            ...
            if (json.followed_by.page_info.has_next_page) {
              return new Promise(
                resolve => {
                  ...
                  resolve(
                    self.getUserFollowers(userId, "after", after + ",20", 1, 1)
                  );
                },
                reject =>
                  console.log(
                    "Error handle response from instagram server(get followers request)"
                  )
              );
            } else {
        ...
        for (var i = 0; i < keys.length; i++) {
          ...
          if (!this.essentialValues[key])
            for (let c in cookies)
              if (
                cookies[c].includes(key) &&
                !cookies[c].includes(key + '=""')
              ) {
                var cookieValue = cookies[c]
                  .split(";")[0]
                  .replace(key + "=", "");
  ...
  auth(username, password) {
    ...
    return fetch("https://www.instagram.com/accounts/login/ajax/", options)
      ...
      .catch(e => {
        console.log(e);
        console.log(
          "Instagram authentication failed (challenge required error)"
        );
      });
  }
  ...
  follow(userId, isUnfollow) {
    const headers = {
      ...
      cookie:
        ...
        "; csrftoken=" +
        this.csrfToken +
        "; mid=WPL0LQAEAAGG3XL5-xHXzClnpqA3; rur=ASH; mid=WRN1_AAEAAE07QksztCl3OCnLj8Y;"
    };

    return fetch(
      "https://www.instagram.com/web/friendships/" +
        userId +
        (isUnfollow == 1 ? "/unfollow" : "/follow"),
      {
        method: "post",
  ...
  getUserDataById(id) {
    let query =
      "ig_user(" +
      id +
      "){id,username,external_url,full_name,profile_pic_url,biography,followed_by{count},follows{count},media{count},is_private,is_verified}";
  ...
  getFeed(items, cursor) {
    ...
    return fetch(
      "https://www.instagram.com/graphql/query/?query_id=17866917712078875&fetch_media_item_count=" +
        items +
